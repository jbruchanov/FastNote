 ...
 * This class doesn't have any explicit tests, beacuse it was refactored in the
 * middle of developping => tested through other tests
 * 
 * @author Joe Scurab
 * 
 */
public abstract class IdeaActivityPresenter extends BasePresenter implements
	...
	mContext.getNameRecordButton().setOnClickListener(
		new OnClickListener() {
		    @Override
		    public void onClick(View v) {
			onNameToText();
	...
	if (src == I.Constants.VOICE_RECOGNITION_REQUEST_CODE_NAME)
	    intent.putExtra(RecognizerIntent.EXTRA_PROMPT,
		    getString(R.string.txtIdeaRecorderName));
     ...
     * 
     * @param directory
     *            {@link Environment.#DIRECTORY_PICTURES} for example
     * @param ie
     *            ".3gp", can be null
     * @return
     */
	...
	return ira.getMediaFolder(directory) + "/" + System.currentTimeMillis()
		+ (ext == null ? "" : ext);
    }

    protected String getIdeaName() {
	return StringTools.nullIfTrimmedEmpty(mContext.getNameEditText()
