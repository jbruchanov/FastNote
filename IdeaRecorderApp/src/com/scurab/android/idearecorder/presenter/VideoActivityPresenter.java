...
public class VideoActivityPresenter extends PhotoActivityPresenter {
	...
	if (!new File(i.getPath()).exists())
	    ...
	    mContext.getPhotoButton().setImageResource(
		    R.drawable.ico_player_play);
	}
	...
	startActivityForResult(
		Intent.createChooser(intent, getString(R.string.lblVideo)),
		I.Constants.REQUEST_TAKE_VIDEO);
    }
	...
	final ProgressDialog pd = ProgressDialog.show(mContext,
		getString(R.string.lblPlzWait), getString(R.string.lblSaving));
	...
	IdeaRecorderApplication ira = (IdeaRecorderApplication) getContext()
		.getApplicationContext();
	return ira.getMediaFolder(Environment.DIRECTORY_MOVIES)
		+ "/_temp_video.mp4";
    }
