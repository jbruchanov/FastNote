...
public class AudioRecordingWidget extends LinearLayout {
    ...
    protected void init() {
	mContentView = View.inflate(getContext(),
		R.layout.audiorecordingwidget, null);
	...
	mRecorder = ar;
	mRecorder
		.setOnStopPlayingListener(new IAudioRecorder.OnStopPlayingListener() {
		    @Override
		    public void onStop() {
			setState(STATE_WAITING_FOR_PLAY_OR_RECORD);
			mTimeHandler.stop();
	...
	public void onTick() {
	    counter++;
	    String time = String.format("%02d:%02d", counter / SEC, counter
		    % SEC);
	    mTime.setText(time);
	}
    }
