...
public class MainActivityPresenterTest extends AndroidTestCase {
	...
	assertNotNull(((HelpImageButton) ma.getConfigButton())
		.getOnClickListener());
	...
	assertEquals(i.getId(),
		ma.startIntent.getLongExtra(I.Constants.IDEA_ID, 0));
    }

    public void testSendByContextMenu() throws IOException,
	    InterruptedException {
	int[] types = new int[] { Idea.TYPE_AUDIO, Idea.TYPE_IMAGE,
		Idea.TYPE_TEXT, Idea.TYPE_VIDEO };
	String[] mimes = new String[] { I.MimeType.AUDIO_3GPP,
		I.MimeType.IMAGE_JPEG, I.MimeType.TEXT_PLAIN,
		I.MimeType.VIDEO_MP4 };
	...
	for (int i = 0; i < types.length; i++) {
	    ...
	    Intent extraIntent = (Intent) ma.startIntent.getExtras().get(
		    Intent.EXTRA_INTENT);
	    ...
	    assertTrue(extra.contains(data.getName())
		    && extra.contains(data.getDescription()));
	    if (types[i] != Idea.TYPE_TEXT) {
		...
		assertEquals("file://" + data.getPath(), extraIntent
			.getExtras().get(Intent.EXTRA_STREAM).toString());
	    }
	    ma.startIntent = null;
	    db.deleteAllData();
	}
	...
	assertNotNull(ma.startIntent);
	assertEquals(PreferencesActivity.class.getName(), ma.startIntent
	...
	@Override
	public void setOnActivityKeyDownListener(
		OnActivityKeyDownListener onActivityKeyDownListener) {
	...
	@Override
	public void setOnContextMenuCreateListener(
	...
	@Override
	public void setOnActivityStateChangeListener(
