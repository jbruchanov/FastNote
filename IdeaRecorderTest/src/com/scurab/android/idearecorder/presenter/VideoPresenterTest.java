...
//		assertTrue(par.startIntent.getExtras().get(MediaStore.EXTRA_OUTPUT).toString().length() > Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MOVIES).length());
	}
...
//		
//		assertTrue(par.onActivityResult(I.Constants.REQUEST_TAKE_VIDEO, Activity.RESULT_OK, new Intent()));
...
//		assertTrue(i.getSaveTime() > 0);
	...
	public void testOpenExistingAndOpenVideo() throws IOException
	{
		...
		assertEquals(i.getDescription(),mpa.getDescriptionEditText().getText().toString());
...
//		assertEquals(1,par.startIntent.getIntExtra(MediaStore.EXTRA_VIDEO_QUALITY, -1));		
